                            ...
                            <table id="zero_config" class="table table-bordered table-striped table-hover dataTable js-exportable">
                                ...
                                <tr>
                                    <th>Action</th><td class="text-center">
                                        <a href="{{ route('admin.images.show',$category->id) }}" class="btn btn-info waves-effect">
                                            <i class="fa fa-eye"></i>
                                        </a>
                                        <a href="{{ route('admin.images.edit',$category->id) }}" class="btn btn-success waves-effect">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                        <button class="btn btn-danger waves-effect" type="button" onclick="deleteService({{ $category->id }})">
                                            <i class="fa fa-trash"></i>
                                        </button>
                                        <form id="delete-form-{{ $category->id }}" action="{{ route('admin.images.destroy',$category->id) }}" method="POST" style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
